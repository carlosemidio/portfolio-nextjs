...
class PostList extends Component<Props, IState> {
  static async getInitialProps({ query }) {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/posts/category/${query.slug}/?from=0&pages=10`
    );
  ...
  async getPosts() {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/posts/category/${this.props.slug}/?from=${this.state.posts.length}&pages=${this.state.pages}`
    );
    const posts = await res.json();
...
//   const res = await fetch(
//     `${process.env.NEXT_PUBLIC_API_URL}/posts/category/${slug}/?from=0&pages=10`
//   );
